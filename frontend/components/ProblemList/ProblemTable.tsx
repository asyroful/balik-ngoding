...
export default function ProblemTable({ problems, onRowClick }: ProblemTableProps) {
  ...
  const isLocked = (problem: Problem) =>
    problem.prerequisiteId !== null && !isAccepted(problem.prerequisiteId);
    ...
    <>
      ...
      <div className="flex flex-col gap-2 md:hidden">
        {problems.map((problem, index) => (
          <div
            key={problem.id}
            onClick={() => !isLocked(problem) && onRowClick(problem.id)}
            className={`rounded-xl border border-gray-200 bg-white px-4 py-3 transition ${isLocked(problem) ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:bg-indigo-50/50 active:bg-indigo-100'}`}
          >
          ...
          <tbody className="divide-y divide-gray-100">
            {problems.map((problem, index) => (
              <tr
                key={problem.id}
                onClick={() => !isLocked(problem) && onRowClick(problem.id)}
                className={isLocked(problem) ? 'cursor-not-allowed opacity-50' : 'cursor-pointer transition hover:bg-indigo-50/50 group'}
              >
